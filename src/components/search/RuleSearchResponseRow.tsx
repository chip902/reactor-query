...
import {
    RuleSearchResponseItem, RuleComponentSearchResponseItem
} from '@/lib/types';
                        ...
                        <div key={component.id} className="p-2 bg-gray-50 rounded-md">
                            <div className="flex items-center justify-between">
                                <span className="text-sm font-medium text-gray-900 pb-2">
                                    ...
                                    {hasSearchMatch(component.attributes, searchValue) && (
                                    ...
                                    >
                                        ...
                                        >
                                            <path
                                                ...
                                                strokeWidth={2}
                                                d={expandedComponents.has(component.id)
                                    ...
                                    <div
                                        dangerouslySetInnerHTML={{
                                            __html: highlightSearchInJson(
                                                formatAttributesWithParsedSettings(component.attributes),
                                                searchValue
                                            )
                                        }}
                                    />
        ...
        <div className="w-full border rounded-lg shadow-sm bg-white mb-2">
                ...
                <div className="flex flex-col space-y-2 w-full sm:flex-row sm:space-y-0 sm:items-center sm:justify-between">
                    <div className="flex items-center space-x-3">
                        <span className="font-medium">
                            {index + 1}. {" "}
                            <a target="_blank" className="text-blue-500 underline hover:text-blue-800" href={`https://experience.adobe.com/#/@organizationName/sname:prod/data-collection/tags/companies/${lastSearchedCompany.id}/properties/${item.relationships.property.data.id}/rules/${item.id}`}>
                                {item.attributes.name}

                            </a>
                            {hasSearchMatch(item, searchValue) && (
                                <span className="ml-2 text-xs no-underline">
                                    🎯
                                </span>
                            )}
                        </span>
                        <span className={`px-2 py-1 text-xs rounded-full ${item.attributes.enabled
                            ? 'bg-green-100 text-green-700'
                            : 'bg-gray-100 text-gray-700'
                            }`}>
                            {item.attributes.enabled ? 'Enabled' : 'Disabled'}
                        </span>
                        {item.attributes.deleted_at && (
            ...
            {isExpanded && (
                <div className="px-4 py-3 border-t">
                    <div className="mb-4">Rule Details
