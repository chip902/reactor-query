...
const DataElementSearchResponseRow = ({
    item,
    searchValue,
    index,
    ...
    const revisionText = item.attributes.revision_number > 0
        ? item.attributes.revision_number
        : 'Current';
                ...
                <div className="flex flex-col space-y-2 w-full sm:flex-row sm:space-y-0 sm:items-center sm:justify-between">
                    <div className="flex items-center space-x-3">
                        <span className="font-medium">
                            {index + 1}. {" "}
                            <a target="_blank" className="text-blue-500 underline hover:text-blue-800" href={`https://experience.adobe.com/#/@organizationName/sname:prod/data-collection/tags/companies/${lastSearchedCompany.id}/properties/${propertyId}/dataElements/${item.id}`}>
                            ...
                            <div
                                dangerouslySetInnerHTML={{
                                    __html: highlightSearchInJson(
                                        formatAttributesWithParsedSettings(item.attributes),
                                        searchValue
                                    )
                                }}
                            />
