...
export async function POST(req: NextRequest) {
	return createReactorRoute<TruncatedReactorAPIResponseItem[], ListRulesRequest>(req, async (reactor, postData) => {
		if (!postData) {
			throw new Error("Missing required property data");
		}
		const allRules = [];
		let rules: ReactorAPIResponse = await reactor.listRulesForProperty(postData.propertyId, { "page[size]": 1000 });
		let { next_page } = rules.meta.pagination;
		allRules.push(...rules.data);

		while (next_page) {
			rules = await reactor.listRulesForProperty(postData.propertyId, { "page[size]": 1000, "page[number]": next_page });
			allRules.push(...rules.data);
